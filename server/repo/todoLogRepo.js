...
function getTodoLogs() {
    ...
    FROM TodoLog
    LEFT JOIN (SELECT id, title FROM TodoSection) as tds
    ON tds.id = TodoLog.todoSectionId
    LEFT JOIN (SELECT id, title FROM TodoCard) as tdc
...
function setTodoLog(action, todoCardId, todoSctionId, timestamp ){
    return (`
